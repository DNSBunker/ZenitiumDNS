...
#pragma warning disable CA1416 // Validate platform compatibility
    public class HttpsClientConnection : DnsClientConnection
    {
        ...
        private async Task<HttpRequestMessage> GetHttpRequestAsync(byte[] requestBuffer, int requestLength, CancellationToken cancellationToken)
        {
            ...
            httpRequest.Content = new ByteArrayContent(requestBuffer, 0, requestLength);
...
public override async Task<DnsDatagram> QueryAsync(DnsDatagram request, int timeout, int retries, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                requestBuffer = System.Buffers.ArrayPool<byte>.Shared.Rent(sendBufferSize);
                int requestLength;

                using (MemoryStream mS = new MemoryStream(requestBuffer, 0, requestBuffer.Length))
                {
                    request.WriteTo(mS);
                    requestLength = (int)mS.Position;
                }
                ...
                while (retry < retries) //retry loop
                {
                    ...
                    using (CancellationTokenSource timeoutCancellationTokenSource = new CancellationTokenSource())
                    {
                        ...
                        {
                            task = _httpClient.SendAsync(await GetHttpRequestAsync(requestBuffer, requestLength, timeoutCancellationTokenSource.Token), timeoutCancellationTokenSource.Token);
                    ...
                    catch (HttpRequestException ex)
                    {
                        if ((retry == 1) && (_udpTunnelProxy is not null))
                        {
                            ...
                            else if (ex.InnerException is System.Net.Quic.QuicException ex2)
                            {
                                switch (ex2.QuicError)
                                {
                                    case System.Net.Quic.QuicError.ConnectionIdle:
                                        //close existing connection to allow reconnection later
                                        _udpTunnelProxy.Dispose();
                    ...
                    {
                        DnsDatagram response = DnsDatagram.ReadFrom(responseStream);

                        response.SetMetadata(_server, Stopwatch.GetElapsedTime(startTimestamp).TotalMilliseconds);
            ...
            finally
            {
                if (requestBuffer is not null)
                    System.Buffers.ArrayPool<byte>.Shared.Return(requestBuffer);
            }
        }
