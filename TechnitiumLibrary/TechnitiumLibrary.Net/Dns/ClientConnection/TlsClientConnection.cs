...
namespace TechnitiumLibrary.Net.Dns.ClientConnection
{
    public class TlsClientConnection : TcpClientConnection
    {
        ...
        { 
            _sslOptions = new SslClientAuthenticationOptions() { TargetHost = _server.Host };
        }
        ...
        protected override async Task<Stream> GetNetworkStreamAsync(Socket socket, CancellationToken cancellationToken)
        {
            ...
            {
                timeoutCts.CancelAfter(TLS_WAIT_TIMEOUT);
                await tlsStream.AuthenticateAsClientAsync(_sslOptions, timeoutCts.Token);
            }
