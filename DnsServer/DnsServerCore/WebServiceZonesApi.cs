...
namespace DnsServerCore
{
    public partial class DnsWebService
    {
        class WebServiceZonesApi
        {
            ...
            private async Task<List<DnsResourceRecord>> ReadRecordsToImportFromAsync(string zoneName, AuthZoneType zoneType, bool overwrite, TextReader zoneFile)
            {
                ...
                switch (type)
                {
                    ...
                    case AuthZoneType.Forwarder:
                        if (request.HasFormContentType && (request.Form.Files.Count > 0))
                        {
                            ...
                            {
                                importRecords = await ReadRecordsToImportFromAsync(zoneName, type, false, zoneFile);
                            }
                        }
            ...
            public async Task ImportZoneAsync(HttpContext context)
            {
                ...
                {
                    records = await ReadRecordsToImportFromAsync(zoneInfo.Name, zoneInfo.Type, overwrite, zoneFile);
                }
            ...
            public void ResyncZone(HttpContext context)
            {
                ...
                switch (zoneInfo.Type)
                {
                    ...
                    default:
                        throw new DnsWebServiceException("Only Secondary, Secondary Forwarder, and Stub zones support resync.");
                }
            }
