...
#pragma warning disable CA1416 // Validate platform compatibility
    ...
    public sealed class DnsServer : IAsyncDisposable, IDisposable, IDnsClient
    {
        ...
        public static readonly Zenitium.Dns.Security.ZenitiumFortressPreProcessor SecurityShield = new();        
        ...
        private async Task ProcessStreamRequestAsync(Stream stream, MemoryStream writeBuffer, SemaphoreSlim writeSemaphore, IPEndPoint remoteEP, DnsDatagram request, DnsTransportProtocol protocol)
        {
            try
            {
                if (request != null && request.Question.Count > 0)
                {
                    var question = request.Question[0];
                    IPAddress clientIp = remoteEP.Address;
                    string domain = question.Name;
        ...
        private async Task<DnsDatagram> DefaultRecursiveResolveAsync(DnsQuestionRecord question, NetworkAddress eDnsClientSubnet, IDnsCache dnsCache, bool dnssecValidation, bool skipDnsAppAuthoritativeRequestHandlers, CancellationToken cancellationToken = default)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (_qnameMinimization && response is not null && response.RCODE == DnsResponseCode.ServerFailure)
                    {
                        _resolverLog?.Write("QNAME Minimization resulted in ServerFailure for '" + question.Name + "'. Initiating fallback to full QNAME...");
                        
                        return await TechnitiumLibrary.TaskExtensions.TimeoutAsync(delegate (CancellationToken cancellationToken1)
                        {
                            return DnsClient.RecursiveResolveAsync(question, dnsCache, _proxy, _preferIPv6, _udpPayloadSize, _randomizeName, false, dnssecValidation, eDnsClientSubnet, _resolverRetries, _resolverTimeout, _resolverConcurrency, _resolverMaxStackCount, true, true, null, cancellationToken1);
                        }, RECURSIVE_RESOLUTION_TIMEOUT, cancellationToken);
                    }
                ...
                {
                    if (_qnameMinimization)
                    {
                        _resolverLog?.Write("QNAME Minimization failed for '" + question.Name + "' (" + ex.Message + "). Initiating fallback to full QNAME...");
